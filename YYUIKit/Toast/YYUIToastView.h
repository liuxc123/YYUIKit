...
@class YYUIToastAnimator;

typedef NS_ENUM(NSInteger, YYUIToastViewPosition) {
    YYUIToastViewPositionTop,
    YYUIToastViewPositionCenter,
    YYUIToastViewPositionBottom
};

/**
 * `YYUIToastView`是一个用来显示toast的控件，其主要结构包括：`backgroundView`、`contentView`，这两个view都是通过外部赋值获取，默认使用`YYUIToastBackgroundView`和`YYUIToastContentView`。
 *
 * 拓展性：`YYUIToastBackgroundView`和`YYUIToastContentView`是YYUI提供的默认的view，这两个view都可以通过appearance来修改样式，如果这两个view满足不了需求，那么也可以通过新建自定义的view来代替这两个view。另外，YYUI也提供了默认的toastAnimator来实现ToastView的显示和隐藏动画，如果需要重新定义一套动画，可以继承`YYUIToastAnimator`并且实现`YYUIToastAnimatorDelegate`中的协议就可以自定义自己的一套动画。
 *
 * 样式自定义：建议通过 tintColor 统一修改整个 toastView 的内容样式。当然你也可以单独修改 contentView.tintColor。默认情况下 YYUIToastView.tintColor = UIColorWhite。
 *
 * 建议使用`YYUIToastView`的时候，再封装一层，具体可以参考`YYUITips`这个类。
 *
 * @see YYUIToastBackgroundView
 * @see YYUIToastContentView
 * @see YYUIToastAnimator
 * @see YYUITips
 */
@interface YYUIToastView : UIView
...
/// @warning 如果使用 [YYUITips showXxx] 系列快捷方法来显示 tips，willShowBlock 将会在 show 之后才被设置，最终并不会被调用。这种场景建议自己在调用 [YYUITips showXxx] 之前执行一段代码，或者不要使用 [YYUITips showXxx] 的方式显示 tips
...
/**
 * `YYUIToastAnimator`可以让你通过实现一些协议来自定义ToastView显示和隐藏的动画。你可以继承`YYUIToastAnimator`，然后实现`YYUIToastAnimatorDelegate`中的方法，即可实现自定义的动画。如果不赋值，则会使用`YYUIToastAnimator`中的默认动画。
 */
@property(nullable, nonatomic, strong) YYUIToastAnimator *toastAnimator;

/**
 * 决定YYUIToastView的位置，目前有上中下三个位置，默认值是center。
 
 * 如果设置了top或者bottom，那么ToastView的布局规则是：顶部从marginInsets.top开始往下布局(YYUIToastViewPositionTop) 和 底部从marginInsets.bottom开始往上布局(YYUIToastViewPositionBottom)。
 */
@property(nonatomic, assign) YYUIToastViewPosition toastPosition;
...
/**
 * `contentView`下面的黑色背景UIView，默认使用`YYUIToastBackgroundView`实现，可以通过`YYUIToastBackgroundView`的 cornerRadius 和 styleColor 来修改圆角和背景色。
 */
...
@interface YYUIToastView (ToastTool)
 ...
 *
 * @param view ToastView 的 superView
 * @return 包含所有 view 里面的所有 YYUIToastView，如果 view = nil 则返回所有内存中的 ToastView
 */
