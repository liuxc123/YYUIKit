...
- (void)layoutSubviews {
    ...
    if ([YYTextKeyboardManager defaultManager].isKeyboardVisible) {
        ...
        CGRect parentViewRect = [[YYTextKeyboardManager defaultManager].keyboardWindow convertRect:self.parentView.frame fromView:self.parentView.superview];
        CGRect intersectionRect = CGRectIntersection(keyboardFrame, parentViewRect);
        BOOL intersectionRectIsValidated = !CGRectIsNull(intersectionRect) && !CGRectIsInfinite(intersectionRect);
        CGRect overlapRect = intersectionRectIsValidated ? intersectionRect : CGRectZero;
