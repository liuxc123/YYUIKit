//
//  YYUIAppBarNavigationController.m
//  YYUIKit
//
...
@interface YYUIAppBarNavigationControllerInfo : NSObject

@property(nonatomic, strong) YYUIAppBarViewController *appBarViewController;
...
@interface YYUIAppBarNavigationController () <UIGestureRecognizerDelegate>
// Whether injected app bars should be hidden.
@property(nonatomic, assign) BOOL appBarHidden;
@end

@implementation YYUIAppBarNavigationControllerInfo
...
@implementation YYUIAppBarNavigationController
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
  ...
  if (self) {
    [self YYUIAppBarNavigationController_commonInit];
  }
  return self;
}
...
- (void)YYUIAppBarNavigationController_commonInit {
...
- (UIViewController *)childViewControllerForStatusBarStyle {
  ...
  YYUIAppBarViewController *appBarViewController = [self appBarViewControllerForViewController:child];
...
- (void)appbar_setNavigationBarHidden:(BOOL)navigationBarHidden
  ...
  if (appBarViewController.headerView.shiftBehavior == YYUIFlexibleHeaderShiftBehaviorDisabled) {
    appBarViewController.headerView.shiftBehavior = YYUIFlexibleHeaderShiftBehaviorHideable;
  }

  // Only toggle visiblity on app bars that have the correct shift behavior enabled.
  if (appBarViewController.headerView.shiftBehavior != YYUIFlexibleHeaderShiftBehaviorHideable) {
    return;
  }
...
- (void)injectAppBarIntoViewController:(UIViewController *)viewController {
  ...
  if ([self.delegate respondsToSelector:@selector
    ...
    YYUIAppBarViewController *appBarViewController = [[YYUIAppBarViewController alloc] init];
  ...
  //    unregistered.
  YYUIAppBarNavigationControllerInfo *info = [[YYUIAppBarNavigationControllerInfo alloc] init];
  info.appBarViewController = appBarViewController;
  info.trackingScrollView = trackingScrollView;
  [self setInfo:info forViewController:viewController];

  if (@available(iOS 11.0, *)) {
    appBarViewController.headerView
  ...
  appBarViewController.topLayoutGuideViewController = viewController;
  ...
  appBarViewController.headerView.observesTrackingScrollViewScrollEvents = YES;

  appBarViewController.traitCollectionDidChangeBlock =
  ...
  if ([self.delegate
    ...
    [self.delegate appBarNavigationController:self
                  willAddAppBarViewController:appBarViewController
  ...
  [viewController addChildViewController:appBarViewController];
  [appBarViewController addSubviewsToParent];
...
- (BOOL)viewControllerHasFlexibleHeader:(UIViewController *)viewController {
  ...
  // case. YYUIFlexibleHeaderViewController can never be a top-level view controller.
  for (UIViewController *childViewController in viewController.childViewControllers) {
    if ([childViewController isKindOfClass:[YYUIFlexibleHeaderViewController class]]) {
      return YES;
    }
...
- (YYUIAppBarNavigationControllerInfo *)infoForViewController:(UIViewController *)viewController {
  return objc_getAssociatedObject(viewController, _cmd);
}

- (void)setInfo:(YYUIAppBarNavigationControllerInfo *)info
    forViewController:(UIViewController *)viewController {
...
- (YYUIAppBarViewController *)appBarViewControllerForViewController:
    (UIViewController *)viewController {
  return [self infoForViewController:viewController].appBarViewController;
}
