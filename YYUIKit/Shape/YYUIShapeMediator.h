...
/*
 ...
 custom shape, your view must have these APIs internally set and get from YYUIShapeMediator's
 corresponding shapedBackgroundColor, shapedBorderColor, and shapedBorderWidth properties.

 This class exists to supersede YYUIShapedShadowLayer which previously added shape support by
 overriding the layerClass and subclassing YYUIShadowLayer, which are both behaviors we want to no
 longer support due to their complexity and constraints.
 */
__attribute__((objc_subclassing_restricted)) @interface YYUIShapeMediator : NSObject
...
/**
 ...
 The YYUIShapeMediator class then keeps a weak reference to the layer and adds the required sublayers
