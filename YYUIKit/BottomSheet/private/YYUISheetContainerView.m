...
@interface YYUISheetContainerView () <YYUIDraggableViewDelegate, YYTextKeyboardObserver>

@property(nonatomic) YYUIBottomSheetState sheetState;
...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self) {
    ...
    if (UIAccessibilityIsVoiceOverRunning()) {
      _sheetState = YYUIBottomSheetStateExtended;
    } else {
      _sheetState = YYUIBottomSheetStatePreferred;
    }
...
- (void)updateSheetState {
  if (UIAccessibilityIsVoiceOverRunning()) {
    ...
    // and accessibile.
    self.sheetState = YYUIBottomSheetStateExtended;
  } else {
    CGFloat currentSheetHeight = CGRectGetMaxY(self.bounds) - CGRectGetMinY(self.sheet.frame);
    self.sheetState = (currentSheetHeight >= [self maximumSheetHeight] ? YYUIBottomSheetStateExtended
...
- (CGPoint)targetPoint {
  ...
  if (!self.ignoreKeyboardHeight) {
    CGFloat keyboardOffset = [YYTextKeyboardManager defaultManager].keyboardFrame.size.height;
    bottomY -= keyboardOffset;
  }

  CGPoint targetPoint;
  switch (self.sheetState) {
    case YYUIBottomSheetStatePreferred:
      targetPoint = CGPointMake(midX, bottomY - [self effectiveSheetHeight]);
      break;
    case YYUIBottomSheetStateExtended:
      targetPoint = CGPointMake(midX, bottomY - [self maximumSheetHeight]);
      break;
    case YYUIBottomSheetStateClosed:
...
- (void)sheetBehaviorDidUpdate {
  ...
  if (self.sheetState == YYUIBottomSheetStateClosed &&
    ...
    self.sheetState = YYUIBottomSheetStatePreferred;
  }
}
...
- (void)draggableView:(__unused YYUIDraggableView *)view
  ...
  YYUIBottomSheetState targetState;
  if (self.preferredSheetHeight == [self maximumSheetHeight]) {
    // Cannot be extended, only closed.
    targetState = ((velocity.y > 0 && self.dismissOnDraggingDownSheet) ? YYUIBottomSheetStateClosed
                                                                       : YYUIBottomSheetStatePreferred);
  } else {
    CGFloat currentSheetHeight = CGRectGetMaxY(self.bounds) - CGRectGetMinY(self.sheet.frame);
    if (currentSheetHeight >= self.preferredSheetHeight) {
      targetState = (velocity.y > 0 ? YYUIBottomSheetStatePreferred : YYUIBottomSheetStateExtended);
    } else {
      targetState = ((velocity.y > 0 && self.dismissOnDraggingDownSheet) ? YYUIBottomSheetStateClosed
                                                                         : YYUIBottomSheetStatePreferred);
    }
...
- (void)setSheetState:(YYUIBottomSheetState)sheetState {
