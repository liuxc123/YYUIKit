...
- (nonnull instancetype)initWithContentViewController:
  ...
  if (self = [super initWithNibName:nil bundle:nil]) {
    ...
    if (UIAccessibilityIsVoiceOverRunning()) {
      _state = YYUISheetStateExtended;
    } else {
      _state = YYUISheetStatePreferred;
    }
    _elevation = 16.0;
...
- (void)bottomSheetWillChangeState:(YYUIBottomSheetPresentationController *)bottomSheet
                        sheetState:(YYUISheetState)sheetState {
  _state = sheetState;
...
- (id<YYUIShapeGenerating>)shapeGeneratorForState:(YYUISheetState)state {
  id<YYUIShapeGenerating> shapeGenerator = _shapeGenerators[@(state)];
  if (state != YYUISheetStateClosed && shapeGenerator == nil) {
    shapeGenerator = _shapeGenerators[@(YYUISheetStateClosed)];
  }
...
- (void)setShapeGenerator:(id<YYUIShapeGenerating>)shapeGenerator forState:(YYUISheetState)state {
