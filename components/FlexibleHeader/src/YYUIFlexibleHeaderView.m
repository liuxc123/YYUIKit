...
#import "private/YYUIFlexibleHeaderMinMaxHeight.h"
#import "private/YYUIFlexibleHeaderShifter.h"
#import "private/YYUIFlexibleHeaderTopSafeArea.h"
#import "private/YYUIFlexibleHeaderView+Private.h"
#import "private/YYUIStatusBarShifter.h"
...
const YYUIFlexibleHeaderShiftBehavior YYUIFlexibleHeaderShiftBehaviorDisabled = 0;
const YYUIFlexibleHeaderShiftBehavior YYUIFlexibleHeaderShiftBehaviorEnabled = 1;
const YYUIFlexibleHeaderShiftBehavior YYUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar = 2;
const YYUIFlexibleHeaderShiftBehavior YYUIFlexibleHeaderShiftBehaviorHideable = 3;
...
// KVO contexts
static char *const kKVOContextYYUIFlexibleHeaderView = "kKVOContextYYUIFlexibleHeaderView";

@interface YYUIFlexibleHeaderView () <YYUIStatusBarShifterDelegate,
YYUIFlexibleHeaderTopSafeAreaDelegate,
YYUIFlexibleHeaderMinMaxHeightDelegate>
...
@property(nonatomic, strong) YYUIFlexibleHeaderMinMaxHeight *minMaxHeight;
...
@interface YYUIFlexibleHeaderScrollViewInfo : NSObject
...
@implementation YYUIFlexibleHeaderView {
    ...
    NSMapTable *_trackedScrollViews;  // {UIScrollView:YYUIFlexibleHeaderScrollViewInfo}
    YYUIFlexibleHeaderScrollViewInfo *_trackingInfo;
    ...
    YYUIFlexibleHeaderShifter *_shifter;
    YYUIStatusBarShifter *_statusBarShifter;
    ...
    YYUIFlexibleHeaderShadowIntensityChangeBlock _shadowIntensityChangeBlock;
...
// YYUIFlexibleHeaderConfiguration properties
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self commonYYUIFlexibleHeaderViewInit];
    }
    return self;
}
...
- (void)commonYYUIFlexibleHeaderViewInit {
    _topSafeArea = [[YYUIFlexibleHeaderTopSafeArea alloc] init];
    _topSafeArea.topSafeAreaDelegate = self;
    
    _minMaxHeight = [[YYUIFlexibleHeaderMinMaxHeight alloc] initWithTopSafeArea:_topSafeArea];
    _minMaxHeight.delegate = self;
    
    _wkWebViewClass = NSClassFromString(@"WKWebView");
    
    _shifter = [[YYUIFlexibleHeaderShifter alloc] init];
    _statusBarShifter = [[YYUIStatusBarShifter alloc] init];
    ...
    _headerContentImportance = YYUIFlexibleHeaderContentImportanceDefault;
...
- (void)fhv_setShadowLayer:(CALayer *)shadowLayer
   intensityDidChangeBlock:(YYUIFlexibleHeaderShadowIntensityChangeBlock)block {
...
#pragma mark - YYUIFlexibleHeaderSafeAreas
...
#pragma mark YYUIFlexibleHeaderTopSafeAreaDelegate

- (void)flexibleHeaderSafeAreaTopSafeAreaInsetDidChange:(YYUIFlexibleHeaderTopSafeArea *)safeAreas {
    ...
    if (_shifter.behavior == YYUIFlexibleHeaderShiftBehaviorHideable && _wantsToBeHidden &&
...
- (BOOL)flexibleHeaderSafeAreaIsStatusBarShifted:(YYUIFlexibleHeaderTopSafeArea *)safeAreas {
    return ([self fhv_canShiftOffscreen] &&
            (_shifter.behavior == YYUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar ||
             _shifter.behavior == YYUIFlexibleHeaderShiftBehaviorHideable) &&
            _statusBarShifter.prefersStatusBarHidden);
}

- (CGFloat)flexibleHeaderSafeAreaDeviceTopSafeAreaInset:(YYUIFlexibleHeaderTopSafeArea *)safeAreas {
    return YYUIDeviceTopSafeAreaInset();
}

#pragma mark - YYUIFlexibleHeaderMinMaxHeightDelegate

- (void)flexibleHeaderMaximumHeightDidChange:(YYUIFlexibleHeaderMinMaxHeight *)safeAreas {
    [self fhv_adjustTrackingScrollViewInsetsForTrackingScrollView:_trackingScrollView];
}

- (void)flexibleHeaderMinMaxHeightDidChange:(YYUIFlexibleHeaderMinMaxHeight *)safeAreas {
    [self fhv_updateLayout];
}
...
- (void)fhv_removeInsetsFromScrollView:(UIScrollView *)scrollView {
    ...
    YYUIFlexibleHeaderScrollViewInfo *info = [_trackedScrollViews objectForKey:scrollView];
    if (!info) {
        return;
    }
...
- (CGFloat)fhv_enforceInsetsForScrollView:(UIScrollView *)scrollView {
    ...
    YYUIFlexibleHeaderScrollViewInfo *info = [_trackedScrollViews objectForKey:scrollView];
    if (!info) {
        info = [[YYUIFlexibleHeaderScrollViewInfo alloc] init];
    ...
    if (!YYUIEdgeInsetsEqualToEdgeInsets(scrollView.contentInset, insets)) {
        scrollView.contentInset = insets;
    }
    
    BOOL statusBarIsHidden = [UIApplication yyui_safeSharedApplication].statusBarHidden ? YES : NO;
...
- (CGFloat)fhv_accumulatorMax {
    ...
    CGFloat statusBarHeight = [UIApplication yyui_safeSharedApplication].statusBarFrame.size.height;
...
- (BOOL)fhv_canShiftOffscreen {
    BOOL interactable = ((_shifter.behavior == YYUIFlexibleHeaderShiftBehaviorEnabled ||
                          _shifter.behavior == YYUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar) &&
                         !_trackingScrollView.pagingEnabled);
    BOOL hideable = _shifter.behavior == YYUIFlexibleHeaderShiftBehaviorHideable;
...
- (void)fhv_recalculatePhase {
    ...
    if (topEdge < 0) {
        _scrollPhase = YYUIFlexibleHeaderScrollPhaseShifting;
        ...
        if ([self fhv_shouldCollapseToStatusBar]) {
            ...
            [UIApplication yyui_safeSharedApplication].statusBarFrame.size.height;
    ...
    if (frame.size.height < self.minMaxHeight.maximumHeightWithTopSafeArea) {
        _scrollPhase = YYUIFlexibleHeaderScrollPhaseCollapsing;
    ...
    _scrollPhase = YYUIFlexibleHeaderScrollPhaseOverExtending;
...
- (void)fhv_updateLayout {
    ...
    // YYUIDeviceTopSafeAreaInset returns an incorrect "best guess" value and we end up storing an
    ...
    // When the shift behavior is YYUIFlexibleHeaderShiftBehaviorHideable, we explicitly disable
    ...
    BOOL allowsInteractiveShift = _shifter.behavior != YYUIFlexibleHeaderShiftBehaviorHideable;
...
- (CGFloat)fhv_anchorLength {
    switch (_headerContentImportance) {
        case YYUIFlexibleHeaderContentImportanceDefault:
            return kMaxAnchorLengthFullSwipe;
            
        case YYUIFlexibleHeaderContentImportanceHigh:
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == kKVOContextYYUIFlexibleHeaderView) {
...
- (void)fhv_startObservingContentOffset {
    [self.trackingScrollView addObserver:self
                              forKeyPath:NSStringFromSelector(@selector(contentOffset))
                                 ...
                                 context:kKVOContextYYUIFlexibleHeaderView];
}
...
- (void)setObservesTrackingScrollViewScrollEvents:(BOOL)observesTrackingScrollViewScrollEvents {
    NSAssert(!observesTrackingScrollViewScrollEvents ||
             (self.shiftBehavior == YYUIFlexibleHeaderShiftBehaviorDisabled ||
              self.shiftBehavior == YYUIFlexibleHeaderShiftBehaviorHideable),
...
#pragma mark - YYUIStatusBarShifterDelegate

- (void)statusBarShifterNeedsStatusBarAppearanceUpdate:
(__unused YYUIStatusBarShifter *)statusBarShifter {
...
- (void)statusBarShifter:(__unused YYUIStatusBarShifter *)statusBarShifter
...
- (void)setShiftBehavior:(YYUIFlexibleHeaderShiftBehavior)shiftBehavior {
    NSAssert((self.observesTrackingScrollViewScrollEvents &&
              (shiftBehavior == YYUIFlexibleHeaderShiftBehaviorDisabled ||
               shiftBehavior == YYUIFlexibleHeaderShiftBehaviorHideable)) ||
    ...
    shiftBehavior = [YYUIFlexibleHeaderShifter behaviorForCurrentContextFromBehavior:shiftBehavior];
    
    if (_shifter.behavior == shiftBehavior) {
        return;
    }
    BOOL needsShiftOnScreen = (_shifter.behavior != YYUIFlexibleHeaderShiftBehaviorDisabled &&
                               shiftBehavior == YYUIFlexibleHeaderShiftBehaviorDisabled);
...
- (YYUIFlexibleHeaderShiftBehavior)shiftBehavior {
    return _shifter.behavior;
}

- (void)setBehavior:(YYUIFlexibleHeaderShiftBehavior)behavior {
...
- (void)changeContentInsets:(YYUIFlexibleHeaderChangeContentInsetsBlock)block {
    if (!block) {
        return;
    }
...
#pragma mark - YYUIElevation

- (void)setElevation:(CGFloat)elevation {
    if (YYUICGFloatEqual(elevation, _elevation)) {
        return;
    }
    
    _elevation = elevation;
    [self yyui_elevationDidChange];
}
...
@implementation YYUIFlexibleHeaderScrollViewInfo
@end
